...
def build_filetree(
    ...
    current: str = "ROOT",
    prefix: str = "",
    summary_field_id: str = "summary_iter",
) -> list[str]:
...
class SummarizerIter(Annotator):
    name = "summarizer_iter"
    summary_field_id = "summary_iter"
    checksum_field_id = "summary_iter_checksum"
    ...
    async def generate_summary(
        ...
        if (
            ...
            or summary_iter_checksum != data.get(self.checksum_field_id)
        ):
            ...
            messages.add_system_prompt(
                name=f"summarizer_iter.{subprompt}", previous_summary=previous_summary
            )
            messages.add_user_prompt(
                name=f"summarizer_iter.user",
                document=document,
                context=context,
            ...
            metadatas[self.checksum_field_id] = summary_iter_checksum
            data[self.checksum_field_id] = summary_iter_checksum
